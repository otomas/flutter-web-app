...
import 'vm_fragment_vehicles_archived.dart';

@RoutePage()
class FragmentVehiclesArchived extends StatefulWidget {
  const FragmentVehiclesArchived({
    super.key,
  ...
  @override
  State<FragmentVehiclesArchived> createState() => _FragmentVehiclesArchivedState();
}

class _FragmentVehiclesArchivedState extends WidgetBaseStatefull<FragmentVehiclesArchived, VmFragmentVehiclesArchived> {
  @override
  SystemUiOverlayStyle systemBarBrightness() => UIBrightnessStyle.getInstance().system();

  @override
  VmFragmentVehiclesArchived createViewModel(BuildContext context) => VmFragmentVehiclesArchived(apiService(context), widget.orderByField, widget.orderByDirection);

  @override
  Widget buildWidget(BuildContext context, VmFragmentVehiclesArchived viewModel) => _getBody(context, viewModel);

  @override
  Widget buildWidgetForWeb(BuildContext context, VmFragmentVehiclesArchived viewModel) => _getBodyWeb(context, viewModel);

  Widget _getBody(BuildContext context, VmFragmentVehiclesArchived viewModel) => Padding(
  ...
  Widget _getBodyWeb(BuildContext context, VmFragmentVehiclesArchived viewModel) => Padding(
        padding: const EdgeInsets.symmetric(horizontal: 20),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const SizedBox(height: 20),
            TextBasic(text: 'Arşivdeki Araçlar', color: R.themeColor.secondaryHover, fontFamily: R.fonts.displayBold, fontSize: 24),
            const SizedBox(height: 20),
            IntrinsicHeight(
              child: Row(
                children: [
  ...
  Widget _getSearchField(BuildContext context, VmFragmentVehiclesArchived viewModel) => TextFieldBasic(
  ...
  Widget _getWebList(BuildContext context, VmFragmentVehiclesArchived viewModel) => Expanded(
        child: Padding(
          padding: const EdgeInsets.symmetric(horizontal: 20),
          child: WidgetDataTable(
            ...
            fetcher: (pageSize, sortModel, filterModel, pageToken) async {
              ...
              if (sortModel?.fieldName != null && (sortModel?.fieldName != viewModel.orderByField || direction != viewModel.orderByDirection)) {
                unawaited(router(context).startNewView(route: routergr.FragmentVehiclesArchived(orderByField: sortModel?.fieldName, orderByDirection: direction)));
                return (<Widget>[], '1');
              }
  ...
  Widget _getList(BuildContext context, VmFragmentVehiclesArchived viewModel) => Expanded(
        child: Stack(
          children: [
