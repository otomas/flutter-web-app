...
class VmFragmentVehicleConsignment extends ViewModelBase {
  VmFragmentVehicleConsignment(this.serviceApi, this.orderByField, this.orderByDirection) {
  ...
  List<ModelDataTableColumn> tableData = [
    ...
    ModelDataTableColumn(id: 'plate_number', title: 'Plaka No'),
    ...
    ModelDataTableColumn(id: 'seller.first_name', title: 'Müşteri', size: const FractionalColumnSize(.15)),
  ...
  Future<void> getDataScrollable([bool isPagination = true]) async {
    ...
    notifyListeners();
    await serviceApi.client.getVehiclesConsignment(start: page, length: take, search: searchController.text).then(
      (response) {
