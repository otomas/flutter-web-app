...
import 'vm_fragment_vehicles_purchased.dart';

@RoutePage()
class FragmentVehiclesPurchased extends StatefulWidget {
  const FragmentVehiclesPurchased({
    super.key,
  ...
  @override
  State<FragmentVehiclesPurchased> createState() => _FragmentVehiclesPurchasedState();
}

class _FragmentVehiclesPurchasedState extends WidgetBaseStatefull<FragmentVehiclesPurchased, VmFragmentVehiclesPurchased> {
  @override
  SystemUiOverlayStyle systemBarBrightness() => UIBrightnessStyle.getInstance().system();

  @override
  VmFragmentVehiclesPurchased createViewModel(BuildContext context) => VmFragmentVehiclesPurchased(apiService(context), widget.orderByField, widget.orderByDirection);

  @override
  Widget buildWidget(BuildContext context, VmFragmentVehiclesPurchased viewModel) => _getBody(context, viewModel);

  @override
  Widget buildWidgetForWeb(BuildContext context, VmFragmentVehiclesPurchased viewModel) => _getBodyWeb(context, viewModel);

  Widget _getBody(BuildContext context, VmFragmentVehiclesPurchased viewModel) => Padding(
  ...
  Widget _getBodyWeb(BuildContext context, VmFragmentVehiclesPurchased viewModel) => Padding(
        padding: const EdgeInsets.symmetric(horizontal: 20),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const SizedBox(height: 20),
            TextBasic(text: 'Aldığım Araçlar', color: R.themeColor.secondaryHover, fontFamily: R.fonts.displayBold, fontSize: 24),
            const SizedBox(height: 20),
            IntrinsicHeight(
              child: Row(
                children: [
  ...
  Widget _getSearchField(BuildContext context, VmFragmentVehiclesPurchased viewModel) => TextFieldBasic(
  ...
  Widget _getList(BuildContext context, VmFragmentVehiclesPurchased viewModel) => Expanded(
        child: Stack(
          children: [
  ...
  Widget _getWebList(BuildContext context, VmFragmentVehiclesPurchased viewModel) => Expanded(
        child: Padding(
          padding: const EdgeInsets.symmetric(horizontal: 20),
          child: WidgetDataTable(
            ...
            fetcher: (pageSize, sortModel, filterModel, pageToken) async {
              ...
              if (sortModel?.fieldName != null && (sortModel?.fieldName != viewModel.orderByField || direction != viewModel.orderByDirection)) {
                unawaited(router(context).startNewView(route: routergr.FragmentVehiclesPurchased(orderByField: sortModel?.fieldName, orderByDirection: direction)));
                return (<Widget>[], '1');
              }
