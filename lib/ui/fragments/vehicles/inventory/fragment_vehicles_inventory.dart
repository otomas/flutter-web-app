...
import 'vm_fragment_vehicles_inventory.dart';

@RoutePage()
class FragmentVehicleInventory extends StatefulWidget {
  const FragmentVehicleInventory({
    super.key,
  ...
  @override
  State<FragmentVehicleInventory> createState() => _FragmentVehicleInventoryState();
}

class _FragmentVehicleInventoryState extends WidgetBaseStatefull<FragmentVehicleInventory, VmFragmentVehicleInventory> {
  @override
  SystemUiOverlayStyle systemBarBrightness() => UIBrightnessStyle.getInstance().system();

  @override
  VmFragmentVehicleInventory createViewModel(BuildContext context) => VmFragmentVehicleInventory(apiService(context), widget.orderByField, widget.orderByDirection);

  @override
  Widget buildWidget(BuildContext context, VmFragmentVehicleInventory viewModel) => _getBody(context, viewModel);

  @override
  Widget buildWidgetForWeb(BuildContext context, VmFragmentVehicleInventory viewModel) => _getBodyWeb(context, viewModel);

  Widget _getBody(BuildContext context, VmFragmentVehicleInventory viewModel) => Padding(
  ...
  Widget _getBodyWeb(BuildContext context, VmFragmentVehicleInventory viewModel) => Padding(
        padding: const EdgeInsets.symmetric(horizontal: 20),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const SizedBox(height: 20),
            TextBasic(text: 'Envanterimdeki Araçlar', color: R.themeColor.secondaryHover, fontFamily: R.fonts.displayBold, fontSize: 24),
            const SizedBox(height: 20),
            IntrinsicHeight(
              child: Row(
                children: [
  ...
  Widget _getSearchField(BuildContext context, VmFragmentVehicleInventory viewModel) => TextFieldBasic(
  ...
  Widget _getWebList(BuildContext context, VmFragmentVehicleInventory viewModel) => Expanded(
        child: Padding(
          padding: const EdgeInsets.symmetric(horizontal: 20),
          child: WidgetDataTable(
            ...
            fetcher: (pageSize, sortModel, filterModel, pageToken) async {
              ...
              if (sortModel?.fieldName != null && (sortModel?.fieldName != viewModel.orderByField || direction != viewModel.orderByDirection)) {
                unawaited(router(context).startNewView(route: routergr.FragmentVehicleInventory(orderByField: sortModel?.fieldName, orderByDirection: direction)));
                return (<Widget>[], '1');
              }
  ...
  Widget _getList(BuildContext context, VmFragmentVehicleInventory viewModel) => Expanded(
        child: Stack(
          children: [
