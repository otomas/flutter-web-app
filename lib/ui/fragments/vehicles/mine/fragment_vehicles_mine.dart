...
import 'vm_fragment_vehicles_mine.dart';

@RoutePage()
class FragmentVehiclesMine extends StatefulWidget {
  const FragmentVehiclesMine({
    super.key,
  ...
  @override
  State<FragmentVehiclesMine> createState() => _FragmentVehiclesMineState();
}

class _FragmentVehiclesMineState extends WidgetBaseStatefull<FragmentVehiclesMine, VmFragmentVehiclesMine> {
  @override
  SystemUiOverlayStyle systemBarBrightness() => UIBrightnessStyle.getInstance().system();

  @override
  VmFragmentVehiclesMine createViewModel(BuildContext context) => VmFragmentVehiclesMine(apiService(context), widget.orderByField, widget.orderByDirection);

  @override
  Widget buildWidget(BuildContext context, VmFragmentVehiclesMine viewModel) => _getBody(context, viewModel);

  @override
  Widget buildWidgetForWeb(BuildContext context, VmFragmentVehiclesMine viewModel) => _getBodyWeb(context, viewModel);

  Widget _getBody(BuildContext context, VmFragmentVehiclesMine viewModel) => Padding(
  ...
  Widget _getBodyWeb(BuildContext context, VmFragmentVehiclesMine viewModel) => Padding(
        padding: const EdgeInsets.symmetric(horizontal: 20),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const SizedBox(height: 20),
            TextBasic(text: 'Araçlarım', color: R.themeColor.secondaryHover, fontFamily: R.fonts.displayBold, fontSize: 24),
            const SizedBox(height: 20),
            IntrinsicHeight(
              child: Row(
                children: [
  ...
  Widget _getSearchField(BuildContext context, VmFragmentVehiclesMine viewModel) => TextFieldBasic(
  ...
  Widget _getWebList(BuildContext context, VmFragmentVehiclesMine viewModel) => Expanded(
        child: WidgetDataTable(
          ...
          fetcher: (pageSize, sortModel, filterModel, pageToken) async {
            ...
            if (sortModel?.fieldName != null && (sortModel?.fieldName != viewModel.orderByField || direction != viewModel.orderByDirection)) {
              unawaited(router(context).startNewView(route: routergr.FragmentVehiclesMine(orderByField: sortModel?.fieldName, orderByDirection: direction)));
              return (<Widget>[], '1');
            }
  ...
  Widget _getList(BuildContext context, VmFragmentVehiclesMine viewModel) => Expanded(
